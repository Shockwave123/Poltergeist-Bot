...
// ---------------------------------------------------------------------------
// WhatsApp drops an 8-digit pairing code within about a minute, and the code is
// bound to the socket that requested it - never hand the user a code we cannot
// still back with a live connection.
...
function prepareAuthFiles() {
  ...
  if (configured && sessionManager.isSessionIdValid(configured) && !hasCreds) {
    // Validate the decoded payload BEFORE writing it — a placeholder string, or a
    // creds dump whose device was never registered (`registered:false`), would
    // otherwise poison session/creds.json.
    let decodedOk = false;
    try {
      const raw = sessionManager.decodeSessionId(configured);
      decodedOk = sessionManager.isLinkedCreds(raw);
    } catch (error) { decodedOk = false; }
    if (!decodedOk) {
      console.error('📡 Session: SESSION_ID does not contain a linked account, ignoring it. Pair fresh to get a real session id.');
  ...
  if (!hasCreds) {
    ...
    if (stored && sessionManager.isSessionIdValid(stored)) {
      let storedOk = false;
      try {
        storedOk = sessionManager.isLinkedCreds(sessionManager.decodeSessionId(stored));
      } catch (error) { storedOk = false; }
      if (!storedOk) {
        console.error('📡 Session: database/session.json holds an unlinked session, ignoring it. Pair fresh to get a real session id.');
...
/**
 * Wait until WhatsApp finished the handshake and asked for pairing (its `pair-device`
 * stanza, which is immediately followed by the `qr` event).
 *
 * `requestPairingCode()` in Baileys only fire-and-forgets the `link_code_companion_reg`
 * IQ - it never waits for a server reply, so a resolved promise proves nothing. If the
 * IQ is sent before the server signalled readiness, WhatsApp answers 428/401/400 and the
 * code a user types is rejected ("Couldn't link device"). An open websocket is not
 * enough, which is why every pairing request goes through this gate.
 */
async function waitForPairingReady(sock, timeoutMs = 25000) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    if (!sock || sock !== activeSocket) return false;
    if (sock.__pairingReady) return true;
    await delay(300);
  }
  return Boolean(sock && sock.__pairingReady);
}
...
async function requestPairingCodeFor(rawPhoneNumber) {
  ...
  try {
    ...
    // Sending the pairing IQ before WhatsApp asked for pairing gets the request
    // rejected (428/401/400) while requestPairingCode() still resolves happily,
    // which is why a printed code could never be linked.
    if (!(await waitForPairingReady(sock))) {
      throw new Error('WhatsApp has not finished the connection handshake yet, so no pairing code was requested. Please try again in a few seconds.');
    }
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      try {
        ...
        if (!isSocketOpen(sock)) {
          ...
          if (!(await waitForPairingReady(sock, 20000))) break;
        }
      }
    }
...
async function startBot(reason = 'startup') {
  ...
  try {
    ...
    // Baileys picks the payload for the handshake with `creds.me`, NOT `registered`
    // (Socket/socket.js: `!creds.me ? generateRegistrationNode() : generateLoginNode()`),
    // and `requestPairingCode()` writes `creds.me` *before* WhatsApp confirms the link.
    // An abandoned or failed pairing therefore leaves `{me, registered:false}` on disk
    // and every later socket sends a LOGIN node for a device that was never registered -
    // the phone then shows "Couldn't link device" no matter how often a code is
    // requested. Drop that stale `me` so the registration path is used again. The key
    // material (and `pairingCode`, so a code already being typed on the phone can still
    // be decrypted) is kept, and a properly linked session is never touched.
    if (state.creds.me && !sessionManager.isLinkedCredsObject(state.creds)) {
      console.warn('⚠️ Session: cleared a stale, never-linked pairing state (creds.me) so this boot can register properly.');
      state.creds.me = undefined;
      try {
        await saveCreds();
      } catch (error) {
        console.error('[creds] could not clear the stale pairing state:', error?.message || error);
      }
    }
    ...
    latestQrRaw = '';
    pairingReadyAt = 0;
    // A pairing code is bound to the socket that requested it - the server relays the
    // phone's companion_finish back to that connection - so a code minted for a socket
    // that has since been replaced must never be handed out again.
    ...
    if (state.creds.registered) {
      ...
      // The cached code was just cleared, so the setup page must offer a fresh
      // QR/code instead of advertising an expired one.
      if (authMethod !== 'session-id') authMethod = 'qr';
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (qr) {
        ...
        qrCount += 1;
        // The `qr` event only fires after the server sent its `pair-device` stanza,
        // which is the earliest moment a pairing code may be requested on this socket.
        sock.__pairingReady = true;
