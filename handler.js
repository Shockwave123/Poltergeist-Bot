...
const GROUP_CACHE_MAX = 200; // max entries before we evict oldest

// Periodically wipe the cache to prevent unbounded RAM growth
setInterval(() => {
...
const getCachedGroupMetadata = async (sock, groupId) => {
  try {
    ...
    if (groupMetadataCache.size >= GROUP_CACHE_MAX) {
